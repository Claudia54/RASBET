...
public class ReadJSON {
    private String filename = "src/main/java/com/example/demo/RASBET_DATA/teste.json";
    ...
    public static Game parseGameObject(JSONObject game, int idSport) {
        ...
        while (bookmakersIterator.hasNext() && !sucesso) {
        ...
        Game g;
        if(!(boolean) game.get("completed")){
            String out;
            if(scores != null) {
                out = Game.parseResult2WithDraw(scores);
            }
            else{
                out = "null";
            }
            g = new Game(id, idSport, out, dateGame);
            g.addDraw(draw);
            g.addParticipant(home, oddHome);
            g.addParticipant(away, oddAway);
        }
        else{
            String out = Game.parseResult2WithDraw(scores);
            g = new Game(id, idSport, Game.closed(), out, dateGame);
            g.addDraw(draw);
            g.addParticipant(home, oddHome);
            g.addParticipant(away, oddAway);
        }
